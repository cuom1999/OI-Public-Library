...
| `sources/ioi/1999/lai-yukun-dynamic-programming.tex` | 36 | Reviewed as a LibreOffice article. The main dynamic-programming article, code appendix, and cited-problem appendix are broadly translated, but problem-statement figures and data are missing: the NOI 1997 Blocks sample table is replaced by a spacing-loss note, and the Blocks cuboid, Polygon, and Free Pie diagrams are absent while the text still depends on them. |
...
| `sources/ioi/2001/network-flow-modeling-jiang-peng.tex` | 5 | Reviewed as a short text-extractable Writer article. The Vietnamese covers the network-flow modeling prose for Develop, but omits the raw sample input/output table and several substantive dependency/flow-network diagrams, including the min-cut illustration and capacity-labeled max-flow figure. |
...
| `sources/ioi/2004/splay-tree-yang-siyu.tex` | 9 | Reviewed as a pdfFactory article. The Vietnamese translates the splay-tree prose, operations, amortized proof, Turnover application, references, and appendix file references, but omits the central operation/proof diagrams for Zig/Zag, Zig-Zig/Zag-Zag, Zig-Zag/Zag-Zig, Splay examples, Join, Split, and subtree-rank notation. |
...
| `sources/ioi/2007-gao-yihan-circle-discretization.tex` | 12 | Reviewed as a selectable-text geometry article. The Vietnamese covers the abstract, method, two examples, conclusion, acknowledgments, references, appendix, source snippet, and both original problem statements, but omits all seven substantive figures for rectangle strips, circle-strip decomposition, scan-line intervals, event regions, special points, and arc coverage. |
...
| `sources/ioi/2007-gu-nan-planar-embedding.tex` | 14 | Reviewed as a Writer article. The Vietnamese covers definitions, algorithm outline, `walkup`, `walkdown`, complexity, correctness prose, related problems, and appendix pseudocode, but figures 1--14 are effectively caption-only; those diagrams carry the edge-embedding cases, external-active nodes, reversal direction, virtual edges, and \(K_{3,3}/K_5\) obstruction configurations used in the proof. |
