...
\begin{tikzpicture}[
  x=1cm,y=1cm,
  vtx/.style={circle,draw,fill=white,minimum size=7mm,inner sep=0pt},
  matched/.style={red,very thick},
  free/.style={black,thick}
]
  \node[vtx] (a1) at (0,3) {1};
  \node[vtx] (a2) at (0,2) {2};
  \node[vtx] (a3) at (0,1) {3};
  ...
  \draw[free] (a4) -- (b8);
  \draw[free] (a3) -- (b5);
  \node[below=4mm of a4,anchor=north west] {Trước khi đảo đường tăng};
  \begin{scope}[xshift=5cm]
    ...
    \draw[matched] (c4) -- (d8);
    \draw[free] (c3) -- (d5);
    \node[below=4mm of c4,anchor=north west] {Sau khi đảo đường tăng};
  \end{scope}
\end{tikzpicture}
\caption{Đảo trạng thái trên đường tăng \(2\to5\to1\to7\to4\to8\). Cạnh đỏ là cạnh đang ghép.}
\end{figure}
...
\begin{tikzpicture}[scale=0.72]
  \draw[step=1cm,gray!80] (0,0) grid (6,5);
  \foreach \x in {0.5,1.5,...,5.5} {
    \draw[->,thick] (\x,5.45) -- (\x,5.05);
  }
  \foreach \y in {0.5,1.5,...,4.5} {
    \draw[->,thick] (-0.45,\y) -- (-0.05,\y);
  }
  \foreach \p in {(2.5,3.5),(1.5,2.5),(3.5,2.5),(2.5,1.5)} {
    \node at \p {\(\ast\ast\)};
  }
  \draw[red,very thick] (0,2.5) -- (6,2.5);
  \draw[red,very thick] (2.5,0) -- (2.5,5);
  \node[below=5mm] at (3,0) {Hai tia đỏ phủ cả bốn vật thể, nên đáp án là \(2\).};
\end{tikzpicture}
\caption{Ví dụ phủ đỉnh nhỏ nhất dưới dạng chọn hàng hoặc cột để bắn laser.}
\end{figure}
...
Các slide gốc có nhắc đến mã nguồn đính kèm cho Hopcroft--Karp và KM qua các
liên kết paste:
\begin{itemize}
  \item Hopcroft--Karp: \url{http://paste.ubuntu.com/11980011/};
  \item KM \(O(n^4)\): \url{http://paste.ubuntu.com/11979990/};
  \item KM \(O(n^3)\): \url{http://paste.ubuntu.com/11979986/}.
\end{itemize}
Theo ngoại lệ của dự án, bản dịch không chép lại hay dịch các mẫu mã nguồn;
phần chính ở trên trình bày ý tưởng, định lý và khung cài đặt tự chứa.
