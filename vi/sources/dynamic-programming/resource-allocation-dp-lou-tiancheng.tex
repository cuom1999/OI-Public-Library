...
dung slide thành ghi chú liền mạch, giữ các ví dụ, định dạng vào/ra và công
thức chuyển chính, đồng thời nhấn mạnh trạng thái, khởi tạo và các cách giảm
phạm vi lặp.
\end{abstract}
...
Dòng đầu tiên chứa số máy \(M\) và số công ty con \(N\); tiếp theo là ma trận
mô tả lợi nhuận khi công ty thứ \(i\) được cấp \(j\) máy.
...
Dữ liệu vào trong slide có dạng:
\[
\begin{array}{l}
n\ C\\
C_1\ P_1(0)\ P_1(1)\ \ldots\ P_1(X_1)\\
\ldots\\
C_n\ P_n(0)\ P_n(1)\ \ldots\ P_n(X_n),
\end{array}
\]
trong đó \(0\le X_i\le \lfloor C/C_i\rfloor\).
...
Dữ liệu vào gồm \(A,B,C\), sau đó là \(p_1,p_2,p_3\), tiếp theo là \(N\) và
danh sách \(N\) thời lượng làm việc \(T[i]\). Kết quả cần in ra là số suất ăn
lớn nhất có thể sản xuất mỗi ngày.
...
Khung thuật toán trong slide gồm bốn bước:
\begin{enumerate}
  \item đọc dữ liệu;
  \item dùng tham lam để tính cận trên và một nghiệm khả thi, rồi quyết định
        có cần chạy DP hay không;
  \item chạy quy hoạch động;
  \item in kết quả.
\end{enumerate}
...
Tệp \texttt{INPUT.TXT} bắt đầu bằng \(B\) (\(0\le B\le 5\)), là số loại hàng
cần mua. Mỗi dòng tiếp theo chứa mã hàng \(C\), số lượng \(K\) và đơn giá
\(P\), với \(1\le C\le 999\), \(1\le K\le 5\), \(1\le P\le 999\). Tệp
\texttt{OFFER.TXT} bắt đầu bằng \(S\) (\(0\le S\le 99\)); mỗi dòng mô tả một
gói ưu đãi bằng các cặp \((C,K)\), rồi kết thúc bằng giá gói \(P\)
(\(1\le P\le 9999\)). Tệp \texttt{OUTPUT.TXT} chứa đúng một số: khoản tiền
nhỏ nhất phải trả.
...
Ví dụ về quan hệ vật chính/phụ kiện trong slide:
\[
\begin{array}{c|l}
\text{Vật chính} & \text{Phụ kiện}\\ \hline
...
Tệp \texttt{budget.in} có dòng đầu \(N,m\), trong đó \(N<32000\) là ngân sách
và \(m<60\) là số vật phẩm mong muốn. Mỗi dòng vật phẩm chứa \(v,p,q\), với
\(v<10000\), \(p\in\{1,\ldots,5\}\); nếu \(q=0\) thì vật phẩm là vật chính,
còn nếu \(q>0\) thì đó là phụ kiện thuộc vật chính số \(q\). Tệp
\texttt{budget.out} in giá trị tối đa, nhỏ hơn \(200000\).
