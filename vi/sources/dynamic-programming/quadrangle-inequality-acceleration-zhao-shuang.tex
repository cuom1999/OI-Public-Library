...
\begin{tikzpicture}[scale=1.05,font=\small]
  ...
  \coordinate (B) at (3.8,2.6);
  \coordinate (C) at (4.5,0.2);
  \coordinate (D) at (.6,-.4);
  \draw[thick] (A) -- (B) -- (C) -- (D) -- cycle;
  \draw[blue!65!black,thick] (A) -- (C);
  \draw[red!70!black,thick] (B) -- (D);
  \fill (A) circle (1.5pt) node[above left] {$A:\ w(i,j)$};
  \fill (B) circle (1.5pt) node[above right] {$B:\ w(i',j)$};
  \fill (C) circle (1.5pt) node[below right] {$C:\ w(i',j')$};
  \fill (D) circle (1.5pt) node[below left] {$D:\ w(i,j')$};
  \node[blue!65!black] at (1.9,1.42) {$AC$};
  \node[red!70!black] at (2.65,.82) {$BD$};
\end{tikzpicture}
\caption{Cách nhìn hình học của bất đẳng thức tứ giác: tổng trọng số ở hai đầu đường chéo \(AC\) không lớn hơn tổng trọng số ở hai đầu đường chéo \(BD\).}
\end{figure}
