...
\begin{figure}[ht]
\centering
\begin{tikzpicture}[x=.9cm,y=.65cm,>=Stealth,font=\small]
  \foreach \i in {1,...,6} {
    \draw (\i,0) rectangle ++(1,0.55);
    \node at (\i+0.5,0.28) {$\i$};
  }
  \node[left] at (1,0.28) {$y:$};
  \draw[blue!65!black,thick] (1,-.22) -- (4,-.22);
  \draw[red!70!black,thick] (2,-.48) -- (5,-.48);
  \foreach \i in {1,2,3} {
    \draw[->,blue!65!black,thick] (\i+0.5,-.1) -- (3.5,-.88);
  }
  \foreach \i in {2,3,4} {
    \draw[->,red!70!black,thick] (\i+0.5,-.1) -- (4.5,-1.48);
  }
  \node[blue!65!black] at (3.5,-1.15) {$f(k,1,3)$};
  \node[red!70!black] at (4.5,-1.75) {$f(k,1,4)$};
  \node at (3,-.78) {$c_k=2$};
\end{tikzpicture}
\caption{Hai cửa sổ chuyển liên tiếp khi \(c_k=2\): các trạng thái cũ ở vị trí \(2,3\) được dùng lại, nên cần tránh tính lặp.}
\end{figure}
Đặt
\[
  a_t=f(k-1,x,t)-t+1.
...
\begin{figure}[ht]
\centering
\begin{tikzpicture}[x=.9cm,y=.55cm,>=Stealth,font=\small]
  \draw[->] (0,0) -- (7.2,0) node[right] {chỉ số};
  \draw[->] (0,0) -- (0,4.2) node[above] {$a$};
  \foreach \x/\y/\lab in {1/2.0/a_2,2/2.8/a_3,3/2.2/a_4,4/3.5/a_5,5/2.5/a_6,6/3.2/a_7} {
    \fill[blue!55] (\x,\y) circle (2pt);
    \node[above] at (\x,\y) {$\lab$};
  }
  \draw[thick,blue!55] (2,2.8) -- (4,3.5) -- (6,3.2);
  \draw[red!70!black,thick] (.75,1.75) -- (1.25,2.25);
  \draw[red!70!black,thick] (.75,2.25) -- (1.25,1.75);
  \draw[->,red!70!black,thick] (4.7,4.05) -- (4.1,3.55);
  \node[align=center,anchor=south] at (4.55,4.05) {giữ các giá trị\\giảm dần trong hàng đợi};
\end{tikzpicture}
\caption{Nguyên tắc hàng đợi đơn điệu: phần tử nhỏ hơn và đứng trước một phần tử lớn hơn sẽ không bao giờ là cực đại của cửa sổ.}
\end{figure}
