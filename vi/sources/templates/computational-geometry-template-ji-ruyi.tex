...
% Source-File: 模板 TEMPLATES/计算几何模板 - 吉如一.md
\usepackage{oipl-vn}
...
\origtitle{Computational Geometry Template}
...
int inmid(db k1, db k2, db k3) {return sign(k1 - k3) * sign(k2 - k3) <= 0;} // k3 nằm trong [k1,k2]
struct point {
	db x, y;
...
point proj(point k1, point k2, point q) { // Hình chiếu của q lên đường thẳng k1,k2
	point k = k2 - k1; return k1 + k * (dot(q - k1, k) / k.abs2());
}
point reflect(point k1, point k2, point q) {return proj(k1, k2, q) * 2 - q;}
int clockwise(point k1, point k2, point k3) { // k1 k2 k3: ngược chiều kim đồng hồ 1, cùng chiều -1, ngược lại 0
	return sign(cross(k2 - k1, k3 - k1));
}
int checkLL(point k1, point k2, point k3, point k4) { // Tìm giao điểm của đường thẳng (L) / đoạn thẳng (S) k1,k2 và k3,k4
...
struct line {
	...
	line push() { // Dịch ra ngoài (phía tay trái) một đoạn eps
		const db eps = 1e-6;
...
vector<line> getHL(vector<line> &L) { // Tìm giao nửa mặt phẳng; nửa mặt phẳng có hướng ngược chiều kim đồng hồ, xuất theo thứ tự ngược chiều kim đồng hồ
...
db closepoint(vector<point>&A, int l, int r) { // Cặp điểm gần nhất; cần sắp xếp trước theo tọa độ x
	if (r - l <= 5) {
		db ans = 1e20;
...
int checkposCC(circle k1, circle k2) { // Trả về số tiếp tuyến chung của hai đường tròn
...
vector<point> getCL(circle k1, point k2, point k3) { // Trả về theo hướng k2->k3; nếu tiếp xúc thì trả về hai điểm
	point k = proj(k2, k3, k1.o); db d = k1.r * k1.r - (k - k1.o).abs2();
	if (sign(d) == -1) return {};
	point del = (k3 - k2).unit() * sqrt(max((db)0.0, d)); return {k - del, k + del};
}
vector<point> getCC(circle k1, circle k2) { // Trả về theo chiều ngược kim đồng hồ trên đường tròn k1; nếu tiếp xúc thì trả về hai điểm
...
vector<point> TangentCP(circle k1, point k2) { // Trả về theo chiều ngược kim đồng hồ trên đường tròn k1
...
db getarea(circle k1, point k2, point k3) {
	// Diện tích có hướng của phần giao giữa đường tròn k1 và tam giác k2 k3 k1.o
...
db area(vector<point> A) { // Đa giác biểu diễn bằng vector<point>, theo chiều ngược kim đồng hồ
	db ans = 0;
...
int contain(vector<point>A, point q) { // 2 bên trong, 1 biên, 0 bên ngoài
...
vector<point> ConvexHull(vector<point>A, int flag = 1) { // flag=0 không chặt, flag=1 chặt
...
vector<point> convexcut(vector<point>A, point k1, point k2) {
	// Giữ lại tất cả các điểm sao cho k1,k2,p theo chiều ngược kim đồng hồ
...
int checkPoS(vector<point>A, point k1, point k2) {
	// Đa giác A và đường thẳng (đoạn thẳng) k1->k2 giao nhau chặt; phần chú thích là cho đoạn thẳng
	struct ins {
		point m, u, v;
...
int checkPosFast(vector<point>A, point k1, point k2) { // Kiểm tra nhanh đoạn thẳng có giao chặt với đa giác hay không
...
// Tách bao lồi thành vỏ lồi trên và vỏ lồi dưới. Nên xoay ngẫu nhiên bao lồi một góc để tránh các hoành độ trùng nhau.
// Nên xử lý riêng trường hợp chỉ có một điểm. Bao lồi theo chiều ngược kim đồng hồ.
...
// Cần đảm bảo bao lồi có số điểm >= 3; 2 bên trong, 1 biên, 0 bên ngoài
...
// d là hướng; trả về tiếp điểm phía trên và tiếp điểm phía dưới
...
// Kiểm tra contain trước; trả về theo chiều ngược kim đồng hồ
...
// Khoảng cách trên mặt cầu, tâm là gốc tọa độ, bán kính 1
...
VP getFF(P3 p1, P3 dir1, P3 p2, P3 dir2) { // Trả về một đường thẳng
...
db getV(P3 k1, P3 k2, P3 k3, P3 k4) { // Tính thể tích
...
// Biến một mặt thành dạng (điểm, vector pháp tuyến)
pair<P3, P3> getF(VP F) {
	return mp(F[0], cross(F[1] - F[0], F[2] - F[0]).unit());
}
// Cắt 3D: giữ lại phần dot(dir,x-p)>=0
