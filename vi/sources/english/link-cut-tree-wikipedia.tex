...
\begin{tikzpicture}[
  ...
  subtree/.style={draw,fill=gray!18}
]
  \node[node] (v) at (0,1.4) {$v$};
  \draw (-.55,2.5) .. controls (-.8,2.1) and (-.45,1.8) .. (-.7,1.5)
    .. controls (-.95,1.1) and (-.6,.75) .. (-.8,.35);
  \node at (-1.65,2.2) {$d(P)>v$};
  \node at (-1.65,.55) {$d(P)<v$};

  \node[node] (r) at (3.1,1.85) {$v$};
  \draw (r) -- (2.15,.65);
  \draw (r) -- (4.05,.65);
  \draw[subtree] (1.65,.1) -- (2.15,.95) -- (2.65,.1) -- cycle;
  \draw[subtree] (3.55,.1) -- (4.05,.95) -- (4.55,.1) -- cycle;
  \node at (2.15,-.18) {$d(P)<v$};
  \node at (4.05,-.18) {$d(P)>v$};

  \node[node] (r2) at (6.35,1.85) {$v$};
  \draw (r2) -- (5.4,.65);
  \draw (r2) -- (7.3,.65);
  \draw[subtree] (4.9,.1) -- (5.4,.95) -- (5.9,.1) -- cycle;
  \draw[subtree] (6.8,.1) -- (7.3,.95) -- (7.8,.1) -- cycle;
  \draw[->,thick] (7.3,1.3) to[bend left=35] (6.65,2.08);
\end{tikzpicture}
\caption{Các đỉnh trong cây phụ trợ được sắp theo độ sâu: phần bên trái của \(v\) nông hơn, còn phần bên phải sâu hơn.}
\end{figure}
...
\begin{tikzpicture}[
  ...
  preferred/.style={very thick},
  normal/.style={gray},
  pathptr/.style={dashed,->}
]
  \node[node] (a) at (0,2.7) {$a$};
  \node[node] (b) at (-.9,1.8) {$b$};
  \node[node] (c) at (0,1.8) {$c$};
  \node[node] (d) at (.9,1.8) {$d$};
  \node[node] (e) at (-1.25,.85) {$e$};
  \node[node] (f) at (-.25,.85) {$f$};
  \node[node] (g) at (.55,.85) {$g$};
  \node[node] (h) at (1.25,.85) {$h$};
  \node[node] (i) at (-.45,-.05) {$i$};
  \node[node] (j) at (.05,-.05) {$j$};
  \draw[preferred] (a)--(b)--(e);
  \draw[preferred] (c)--(f)--(i);
  \draw[preferred] (d)--(g);
  \draw[normal] (a)--(c) (a)--(d) (b)--(f) (d)--(h) (f)--(j);
  \node[align=center] at (0,-.85) {cây được biểu diễn\\với cạnh ưu tiên};

  \node[node] (p1a) at (3.1,2.55) {$a$};
  \node[node] (p1b) at (3.1,1.85) {$b$};
  \node[node] (p1e) at (3.1,1.15) {$e$};
  \draw[preferred] (p1a)--(p1b)--(p1e);
  \node[node] (p2c) at (4.3,2.2) {$c$};
  \node[node] (p2f) at (4.3,1.5) {$f$};
  \node[node] (p2i) at (4.3,.8) {$i$};
  \draw[preferred] (p2c)--(p2f)--(p2i);
  \node[node] (p3d) at (5.45,2.2) {$d$};
  \node[node] (p3g) at (5.45,1.5) {$g$};
  \draw[preferred] (p3d)--(p3g);
  \node[node] (p4h) at (6.35,1.5) {$h$};
  \node[node] (p5j) at (4.95,.45) {$j$};
  \draw[pathptr] (p2c) to[bend left=15] (p1a);
  \draw[pathptr] (p3d) to[bend right=15] (p1a);
  \draw[pathptr] (p4h) to[bend left=15] (p3d);
  \draw[pathptr] (p5j) to[bend right=15] (p2f);
  \node[align=center] at (4.75,-.85) {rừng cây phụ trợ\\và con trỏ cha của đường};
\end{tikzpicture}
\caption{Một cây link/cut biến các đường đi ưu tiên thành một rừng cây phụ trợ; các cạnh ngoài đường đi được lưu bằng con trỏ cha của đường.}
\end{figure}

Các thao tác ta quan tâm là
\[
  \texttt{FindRoot(Node v)},\quad
  \texttt{Cut(Node v)},\quad
  \texttt{Link(Node v, Node w)},\quad
  \texttt{Path(Node v)}.
\]
Mọi thao tác đều được cài đặt bằng thủ tục con \texttt{Access(Node v)}. Khi ta truy cập một đỉnh
...
\begin{tikzpicture}[
  ...
  \node[node] (w1) at (0,1.8) {$w$};
  \node[node] (v1) at (1.25,.9) {$v$};
  \draw[preferred] (w1)--(0,.95);
  \draw[normal] (w1)--(-.75,.75);
  \draw[subtree] (-1.15,.15)--(-.75,.85)--(-.35,.15)--cycle;
  \draw[pathptr] (v1) to[bend left=18] (w1);
  \draw[subtree] (.85,.1)--(v1)--(1.65,.1)--cycle;

  \node[node] (w2) at (3.05,1.8) {$w$};
  \node[node] (v2) at (4.25,.9) {$v$};
  \draw[normal] (w2)--(2.35,.75);
  \draw[subtree] (1.95,.15)--(2.35,.85)--(2.75,.15)--cycle;
  \draw[preferred] (w2)--(v2);
  \draw[subtree] (3.85,.1)--(v2)--(4.65,.1)--cycle;

  \node[node] (v3) at (6.7,1.8) {$v$};
  \node[node] (w3) at (6.05,.9) {$w$};
  \draw[preferred] (v3)--(w3);
  \draw[normal] (w3)--(5.55,.1) (w3)--(6.55,.1);
  \draw[subtree] (5.15,-.45)--(5.55,.25)--(5.95,-.45)--cycle;
  \draw[subtree] (6.15,-.45)--(6.55,.25)--(6.95,-.45)--cycle;

  \node[align=center] at (.6,-.9) {trước khi truy cập};
  \node[align=center] at (3.6,-.9) {đổi cạnh ưu tiên};
  \node[align=center] at (6.35,-.9) {splay \(v\) lên gốc};
\end{tikzpicture}
\caption{Trong \texttt{Access}, các đường đi ưu tiên cũ bị phá thành con trỏ cha của đường, còn đỉnh đang truy cập được splay lên gốc cây phụ trợ.}
\end{figure}
