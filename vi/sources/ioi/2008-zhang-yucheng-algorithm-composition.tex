...
\begin{tikzpicture}[
  x=0.72cm,y=0.58cm,
  pt/.style={circle,draw,fill=white,inner sep=1.8pt},
  grid/.style={gray!55,dashed},
  seg/.style={black,very thick}
]
  \foreach \y in {0,1,2,3} \draw[grid] (-0.8,\y)--(6.8,\y);
  \foreach \x in {0,2,2.8,4,6} \draw[grid] (\x,-0.25)--(\x,3.35);
  \foreach \x/\y in {0/3,2/3,4/3,6/3,0/2,2.8/2,4/2,2/1,4/1,6/1,0/0,6/0}
    \node[pt] at (\x,\y) {};
  \draw[seg] (2,3)--(4,3);
  \draw[seg] (2,1)--(4,1);
\end{tikzpicture}\\[-0.2em]
{\small Hình 2.3. Đoạn \([2,4]\) xuất hiện \(2\) lần, tương ứng với hai đoạn
được tô đậm.}
\end{center}
