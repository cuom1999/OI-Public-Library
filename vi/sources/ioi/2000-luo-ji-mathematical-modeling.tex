...
\begin{enumerate}
  \item \term{Thuộc tính} là tính chất không thể thiếu của một sự vật. Trong
  bài này, thuộc tính chỉ mọi đặc trưng cơ chế quyết định nguyên mẫu thông tin.

  \item \term{Bài Xâu 01.} Cho bảy số nguyên
  \(N,A_0,B_0,L_0,A_1,B_1,L_1\). Cần thiết kế một xâu nhị phân
  \[
    S=s_1s_2\cdots s_i\cdots s_N
  \]
  thỏa các điều kiện sau:
  \begin{enumerate}
    \item \(s_i=0\) hoặc \(s_i=1\), với \(1\le i\le N\);
    \item với mọi xâu con liên tiếp độ dài \(L_0\),
    \(s_js_{j+1}\cdots s_{j+L_0-1}\), \(1\le j\le N-L_0+1\), số ký tự \(0\)
    không nhỏ hơn \(A_0\) và không lớn hơn \(B_0\);
    \item với mọi xâu con liên tiếp độ dài \(L_1\),
    \(s_js_{j+1}\cdots s_{j+L_1-1}\), \(1\le j\le N-L_1+1\), số ký tự \(1\)
    không nhỏ hơn \(A_1\) và không lớn hơn \(B_1\).
  \end{enumerate}
  Dữ liệu vào gồm \(N,A_0,B_0,L_0,A_1,B_1,L_1\), với
  \[
    3\le N\le 1000,\qquad
    1\le A_0\le B_0\le L_0\le N,\qquad
    1\le A_1\le B_1\le L_1\le N.
  \]
  Kết quả chỉ gồm một dòng. Nếu không tồn tại xâu 01 thỏa mọi điều kiện, in
  số nguyên \(-1\); ngược lại in một xâu 01 thỏa điều kiện.

  \item Tối ưu hóa trong bài được hiểu là tối ưu theo ba mặt: thời gian, không
  gian và độ phức tạp lập trình.

  \item Toàn bộ bài viết thảo luận việc xây dựng mô hình. Các thuật toán tương
  ứng với mô hình trong phạm vi bài này đều xem như không bao gồm vét cạn. Nếu
  coi vét cạn là một dạng phân tích thống kê thì tập nghiệm thu được \(A\) đã
  bằng toàn bộ tập \(I\), tức bỏ qua bước xây dựng mô hình toán học mà đi thẳng
  tới mục tiêu. Vì vậy thống kê trong bài được hiểu dưới tiền đề \(A\subset I\);
  vét cạn và tìm kiếm kiểu liệt kê không nằm trong phạm vi thảo luận.
\end{enumerate}

\section*{Phụ lục chương trình}

Do giới hạn độ dài, bản gốc chỉ đưa ba chương trình Pascal: bài \textit{Xâu
01} của NOI 1999, bài tìm số lớn thứ \(k\), và bài cực trị của NOI 1995. Theo
ngoại lệ không dịch mẫu mã nguồn của dự án, bản dịch giữ phần thuật toán trong
các mục tương ứng thay vì chép lại toàn bộ mã chương trình. Ba chương trình
trong bản gốc lần lượt hiện thực:
\begin{itemize}
  \item bài \textit{Xâu 01}: lưu dãy tổng tiền tố \(T\) trong mảng
  \texttt{data}, tạo sáu loại cạnh có độ lệch \(1\), \(-1\), \(L_0\),
  \(-L_0\), \(L_1\) và \(-L_1\), cùng trọng số tương ứng, rồi dùng lặp thư giãn
  để tìm các giá trị ngắn nhất;
  \item bài tìm số lớn thứ \(k\): dùng phân hoạch ngẫu nhiên kiểu quicksort,
  nhưng chỉ đệ quy vào phía còn có thể chứa phần tử hạng \(k\);
  \item bài cực trị NOI 1995: sinh các số Fibonacci liên tiếp lớn nhất không
  vượt quá \(k\), rồi in cặp \(M,N\).
\end{itemize}

\section*{Tài liệu tham khảo}

\begin{enumerate}
  \item \textit{Huấn luyện năng lực tư duy toán học}, Nhà xuất bản Nhân dân
  Quảng Đông, 1992.
  \item \textit{Tuyển tập luận văn xuất sắc đội tuyển tập huấn IOI 1998 Trung
  Quốc}, Đội tuyển tập huấn IOI Trung Quốc, ban biên tập
  \textit{Olympic Tin học}, 1999.
  \item Shen Jihong, Shi Jiuyu và cộng sự, \textit{Mô hình hóa toán học}, Nhà
  xuất bản Đại học Công trình Cáp Nhĩ Tân, 1998.
  \item Wang Shuhe, \textit{Cơ sở mô hình toán học}, Nhà xuất bản Đại học Khoa
  học và Công nghệ Trung Quốc, 1997.
  \item Xu Lizhi, \textit{Trừu tượng toán học và phương pháp trừu tượng toán
  học}, 1990.
\end{enumerate}
