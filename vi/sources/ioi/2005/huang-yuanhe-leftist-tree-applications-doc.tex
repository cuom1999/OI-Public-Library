...
Một cây lệch trái có thể được ghi bằng cách gắn mỗi nút với cặp
\((\operatorname{key},\operatorname{dist})\). Sơ đồ dưới đây minh họa ý tưởng
đó: lá có khoảng cách \(0\), còn các nút nằm sâu hơn vẫn thỏa
...
\centering
\begin{tikzpicture}[
  v/.style={circle, draw, minimum size=9mm, inner sep=1pt, align=center},
...
\node[v] {\(1,2\)}
  ...
  child {node[v] {\(3,1\)}
    ...
    child {node[v] {\(18,0\)}}};
\end{tikzpicture}
\caption{Ví dụ cây lệch trái: mỗi nút ghi \((\operatorname{key},\operatorname{dist})\).}
\end{figure}
...
Quá trình gộp cặp đôi biến các cây một nút thành các cây lớn hơn. Về bản chất,
các cây có kích thước gần nhau được gộp theo từng
...
\foreach \i/\x in {1/0,2/0.8,3/1.6,4/2.4,5/3.2,6/4.0,7/4.8,8/5.6} {
  \node[box] (a\i) at (\x,0) {\(\i\)};
}
\foreach \i/\x/\lab in {1/0.4/{1,2},2/2.0/{3,4},3/3.6/{5,6},4/5.2/{7,8}} {
  \node[box] (b\i) at (\x,-1.1) {\(\lab\)};
}
\foreach \i/\x/\lab in {1/1.2/{1..4},2/4.4/{5..8}} {
  \node[box] (c\i) at (\x,-2.2) {\(\lab\)};
}
\node[box] (d) at (2.8,-3.3) {\(1..8\)};
\foreach \i in {1,2} {\draw[arrow] (a\i.south) -- (b1.north);}
\foreach \i in {3,4} {\draw[arrow] (a\i.south) -- (b2.north);}
\foreach \i in {5,6} {\draw[arrow] (a\i.south) -- (b3.north);}
\foreach \i in {7,8} {\draw[arrow] (a\i.south) -- (b4.north);}
\draw[arrow] (b1.south) -- (c1.north);
\draw[arrow] (b2.south) -- (c1.north);
\draw[arrow] (b3.south) -- (c2.north);
\draw[arrow] (b4.south) -- (c2.north);
\draw[arrow] (c1.south) -- (d.north);
\draw[arrow] (c2.south) -- (d.north);
\node[align=center] at (7.0,-1.65)
  {gộp từng cặp\\rồi đưa cây mới\\về cuối hàng đợi};
\end{tikzpicture}
\caption{Xây cây lệch trái bằng cách gộp cặp đôi các cây có kích thước gần nhau.}
\end{figure}
