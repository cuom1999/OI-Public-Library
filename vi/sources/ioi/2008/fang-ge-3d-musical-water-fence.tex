...
nước âm nhạc ba chiều hoàn chỉnh, viết tắt là \term{3dmwf}. 3dmwf là một mảng
gồm \(n\) hàng và \(m\) cột các ô vuông. Ô ở hàng \(i\), cột \(j\) được chiếm
trọn bởi một khối \(b_{ij}\) có chiều cao \(h_{ij}\). Phía trên mỗi khối có
một vòi nước có thể đổ nước xuống.

Ví dụ dưới đây là một mảng \(4\times 4\); số trong mỗi ô là chiều cao của khối
tương ứng.

\begin{center}
\begin{tikzpicture}[
  cell/.style={draw,minimum width=0.78cm,minimum height=0.48cm,inner sep=0pt},
  every node/.style={font=\scriptsize}
]
  \foreach \x/\y/\h in {
    1/4/5,2/4/12,3/4/11,4/4/8,
    1/3/15,2/3/2,3/3/4,4/3/14,
    1/2/16,2/2/1,3/2/3,4/2/7,
    1/1/13,2/1/10,3/1/9,4/1/6
  } {
    \node[cell,fill=black!\h] at (\x*0.78,\y*0.48) {\h};
  }
  \node[anchor=east] at (0.45,1.92) {hàng 1};
  \node[anchor=east] at (0.45,0.48) {hàng 4};
  \node[anchor=north] at (0.78,0.08) {cột 1};
  \node[anchor=north] at (3.12,0.08) {cột 4};
\end{tikzpicture}
\end{center}
