...
nhìn trực tiếp. Điểm mấu chốt trong lời giải là tìm ra một bất biến và mô
...
\begin{tikzpicture}[
  ...
  trie/.style={circle,draw,inner sep=1.7pt,minimum size=6mm},
  edge/.style={-{Latex[length=1.8mm]},thick},
  path/.style={edge,blue!60!black,line width=0.8pt}
]
  \node[trie] (lr) at (0,1.7) {};
  \node[trie] (la) at (-0.8,0.7) {};
  \node[trie] (lb) at (0.4,0.7) {};
  \node[trie] (ll) at (-1.35,-0.35) {};
  \node[trie] (lc) at (-0.25,-0.35) {};
  \draw[edge] (lr) -- node[left] {\(x\)} (la);
  \draw[edge] (lr) -- node[right] {\(u\)} (lb);
  \draw[path] (la) -- node[left] {\(y\)} (ll);
  \draw[edge] (la) -- node[right] {\(z\)} (lc);
  \node[above] at (lr.north) {\(left\_root\)};
  \node[below] at (ll.south) {\(left\_leaf\)};

  \node[trie] (rr) at (4.0,1.7) {};
  \node[trie] (ra) at (3.25,0.7) {};
  \node[trie] (rb) at (4.55,0.7) {};
  \node[trie] (rl) at (4.95,-0.35) {};
  \node[trie] (rc) at (3.0,-0.35) {};
  \draw[edge] (rr) -- node[left] {\(p\)} (ra);
  \draw[path] (rr) -- node[right] {\(q\)} (rb);
  \draw[path] (rb) -- node[right] {\(r\)} (rl);
  \draw[edge] (ra) -- node[left] {\(s\)} (rc);
  \node[above] at (rr.north) {\(right\_root\)};
  \node[below] at (rl.south) {\(right\_leaf\)};

  \draw[-{Latex[length=2mm]},thick] (1.0,0.55) -- node[above] {ghép đường} (3.0,0.55);
  \node[below] at (2.0,-0.75) {chuỗi \(s = yxqr\)};
\end{tikzpicture}
\caption{Sơ đồ left-right tree: phần trái đọc từ lá lên gốc, phần phải đọc từ gốc xuống lá; các nhánh không tô đậm là đoạn dùng chung với những chuỗi khác.}
\end{figure}
...
Bản dịch đã rà lại các đoạn chính về ma trận mẹ, left-right tree, mã ngoặc,
hai dãy DFS, cha nhảy, cây DFS và cây BFS. Sơ đồ left-right tree được dựng lại
từ mô tả trong bài; các hình Word còn lại không để lại tham chiếu treo và nội
dung thuật toán của chúng đã được chuyển thành công thức hoặc văn xuôi. Các phát
biểu đề trong phụ lục dài được tóm lược khi cần cho lập luận, không chép lại
nguyên văn.
