...
\begin{figure}[ht]
\centering
\begin{tikzpicture}[scale=0.72, >=Latex, every node/.style={font=\small}]
\foreach \x in {0,...,5} {
  \foreach \y in {0,...,3} {
    \draw[gray!50] (\x,\y) rectangle ++(1,1);
  }
}
\fill[blue!10] (0,2) rectangle (6,4);
\fill[blue!10] (0,1) rectangle (3,2);
\draw[very thick, red!70!black] (0,2) -- (3,2) -- (3,1) -- (6,1);
\node[blue!60!black] at (1.8,3.55) {đã xử lý};
\node[gray!70!black] at (4.45,0.45) {chưa xử lý};
\foreach \x/\lab in {0.5/1,1.5/2,2.5/0,3.5/1,4.5/0,5.5/2} {
  \draw[->, thick, teal!70!black] (\x,1.98) -- (\x,1.45);
  \node[teal!70!black] at (\x,1.2) {\(\lab\)};
}
\draw[->, thick, teal!70!black] (3.0,1.5) -- (3.55,1.5)
  node[right] {\(1\)};
\node[align=center] at (3,-0.55) {Đường biên chỉ giữ các phích cắm còn ảnh hưởng tới tương lai;\\
các nhãn bằng nhau nghĩa là cùng thành phần liên thông.};
\end{tikzpicture}
\caption{Đường biên và các phích cắm trong quy hoạch động theo ô.}
\end{figure}
...
\begin{figure}[ht]
\centering
\begin{tikzpicture}[x=0.95cm, y=0.7cm, >=Latex, every node/.style={font=\small}]
\foreach \x/\sym in {1/{(},2/{(},3/{)},4/{)},5/{\#},6/{(},7/{)}} {
  \draw (\x,0) circle (0.16);
  \node at (\x,-0.45) {\texttt{\sym}};
}
\draw[thick, blue!70!black] (1,0.1) .. controls (1.6,1.1) and (3.4,1.1) .. (4,0.1);
\draw[thick, blue!70!black] (2,0.1) .. controls (2.25,0.65) and (2.75,0.65) .. (3,0.1);
\draw[thick, orange!85!black] (6,0.1) .. controls (6.25,0.65) and (6.75,0.65) .. (7,0.1);
\node[align=center] at (4,-1.25) {Các cặp đầu mút của đường đi không bắt chéo,\\
nên có thể mã hóa bằng chuỗi ngoặc đúng.};
\end{tikzpicture}
\caption{Biểu diễn ngoặc cho các cặp phích cắm của những đường đi rời nhau.}
\end{figure}
...
\begin{figure}[ht]
\centering
\begin{tikzpicture}[scale=0.58, >=Latex, every node/.style={font=\small}]
\foreach \x in {0,...,5} {
  \foreach \y in {0,...,8} {
...
\foreach \y in {0,...,8} {
  \draw[fill=yellow!30] (-0.45,\y+0.25) rectangle ++(0.25,0.5);
  \draw[fill=red!25] (6.2,\y+0.25) -- ++(0.45,0.25) -- ++(-0.45,0.25) -- cycle;
}
\fill[blue!10] (0,5) rectangle (6,9);
\fill[blue!10] (0,4) rectangle (3,5);
\draw[very thick, red!70!black] (0,5) -- (3,5) -- (3,4) -- (6,4);
\foreach \x/\bit in {0.5/1,1.5/0,2.5/1,3.5/0,4.5/0,5.5/1} {
  \draw[->, thick, teal!70!black] (\x,4.98) -- (\x,4.42);
  \node[teal!70!black] at (\x,4.15) {\(\bit\)};
}
\draw[->, thick, teal!70!black] (3,4.5) -- (3.55,4.5) node[right] {\(1\)};
\node[align=center] at (3,-0.75) {Trong \textit{Rocket Mania}, trạng thái phải nhớ cả liên thông của phích cắm\\
và bit \(fired\): thành phần nào đã được đốt từ que diêm bên trái.};
\end{tikzpicture}
\caption{Mô hình đường biên của \textit{Rocket Mania}: que diêm ở trái, tên lửa
ở phải, còn \(fired\) đánh dấu các thành phần đã được đốt.}
\end{figure}
...
tiếng Anh. Bản dịch giữ nội dung chính, phục dựng các sơ đồ cốt lõi về đường
biên, biểu diễn ngoặc và mô hình \textit{Rocket Mania} bằng TikZ, phục dựng
các công thức chuẩn có thể suy ra từ ngữ cảnh, giữ các bảng số còn đọc được,
và không chép dài các phụ lục đề bài.
