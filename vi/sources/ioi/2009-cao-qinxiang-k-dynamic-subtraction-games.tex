...
\subsection{Ý nghĩa trong thi Tin học}

Bài toán trò chơi vừa kiểm tra nguyên lý cơ bản của quy hoạch động, vừa đòi hỏi
khả năng vận dụng linh hoạt nhiều kiểu phân tích: đơn điệu, đối xứng, tham lam,
công thức số học, hàm SG và mô hình hóa trạng thái. Vì không có một công thức
cố định dùng được cho mọi bài, đây là dạng bài tổng hợp rất thích hợp để kiểm
tra năng lực phân tích. Trong vài năm gần đây, đặc biệt là vào thời điểm bài
viết gốc ra đời, các bài trò chơi xuất hiện ngày càng nhiều ở các kỳ thi quốc
tế và trên các judge lớn, nên có thể xem trò chơi tổ hợp là một hướng nóng mới
trong thi Tin học.
...
Trước hết xét trường hợp chỉ có một quân mã ở \((x,y)\). Vì sau mỗi nước
\(x+y\) đều giảm, đồ thị trạng thái là DAG và có thể dùng quy hoạch động
\(\Oh(N^2)\). Nhưng \(N\) lớn nên lời giải như vậy chỉ dùng để tạo bảng nhỏ và
quan sát quy luật. Các bảng \(N/P\) thay đổi theo \(N\bmod 4\); sau khi tách
theo phần dư, mẫu thắng thua trở nên đều đặn và có thể xác định trong
\(\Oh(1)\). Công thức một quân chưa giải quyết trực tiếp trường hợp nhiều quân,
nhưng nó cho biết đại lượng nào cần đo khi các quân cùng di chuyển theo từng
vòng.
...
Trực giác chiến lược là: với một quân mà mình có thể thắng, người chơi muốn
trì hoãn chiến thắng đó càng lâu càng tốt; với một quân chắc thua, người chơi
muốn kết thúc nó càng nhanh càng tốt. Nếu hai bên đều chỉ có thể thắng ở một
số quân riêng lẻ, quân có thời điểm phân định muộn nhất sẽ là quân quyết định
toàn cục. Dấu của \(RR\) ghi ai thắng trên quân đó, còn trị tuyệt đối ghi độ
muộn của kết cục.
...
Nói cách khác, để thật sự có thời gian \(\Oh(n^3)\), cần chứng minh
\[
  \sum_i |L_A[i]|\cdot |L_A[d-i]|=\Oh(n^3),
\]
và để thật sự có bộ nhớ \(\Oh(n^2)\), cần chứng minh
\[
  \max_i |L_A[i]|\cdot |L_A[d-i]|=\Oh(n^2).
\]
Phản ví dụ ``ống nước và bể nước'' phủ định cả hai ước lượng: các ống bảo đảm
độ dài vào-ra cân bằng, còn các bể tạo ra nhiều ô cùng nằm trên một lớp khoảng
cách, khiến số cặp trạng thái trên những lớp đối xứng tăng vượt xa dự đoán.
...
Kết luận thực tế của ví dụ này không phải là bác bỏ hướng tham lam. Sai sót nằm
ở bước ước lượng số trạng thái của lời giải chính thức: với ràng buộc nhỏ và
hằng số thấp, chương trình kiểu đó vẫn có thể chạy qua nhiều bộ kiểm thử, nhưng
mảng cấp phát theo \(\Oh(n^2)\) có thể tràn trên cấu hình xấu. Điểm cần giữ lại
là tiêu chí ``đến trước'' giúp thoát khỏi DP bốn chiều, còn phần chứng minh độ
phức tạp phải dựa trên cấu trúc lớp đường ngắn nhất sau khi loại bỏ các ô không
thuộc đường ngắn nhất.
