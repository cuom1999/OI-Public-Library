...
\section{Dữ liệu mẫu của bài ghép từ}

Với bài \textit{Play on Words}, mỗi ca kiểm thử gồm số tấm bảng \(N\), sau đó
là \(N\) từ. Cần in ``Ordering is possible.'' nếu có thể sắp xếp tất cả tấm
bảng sao cho chữ đầu của mỗi từ bằng chữ cuối của từ trước nó; ngược lại in
...
\section{Dữ liệu mẫu của bài quản lý kho}

Bài \textit{Depot Rearrangement} yêu cầu tối thiểu hóa số lần chuyển thùng vào
ô trống. Mỗi thao tác được ghi bằng hai vị trí \(x,y\): chuyển thùng ở vị trí
\(x\) sang vị trí trống \(y\). Ban đầu vị trí \(N M+1\) là ô trống.
...
\section{Dữ liệu mẫu của bài máy đánh bạc}

Trong bài \textit{Gambling}, dòng đầu là \(n\). Dòng \(i+1\) chứa \(k_i\) rồi
tới các phần tử của \(S_i\). Nếu máy được cấu tạo sao cho có thể thắng, cần in
một dãy hữu hạn các số được sinh ra và kết thúc bằng \(0\); nếu không, in
``NIE''.
