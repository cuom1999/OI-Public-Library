...
\]
Trong sơ đồ dưới đây, hai đường thẳng có hệ số góc \(t_1,t_2\) kẹp các nhóm
\(Q_1,Q_2,Q_3\), trực quan cho thấy không thể đi tới nửa trục dương của \(Y\).

\begin{center}
\begin{tikzpicture}[>=Latex,scale=0.92,every node/.style={font=\small}]
  \draw[->] (-2.25,0) -- (2.65,0) node[right] {\(X\)};
  \draw[->] (0,-2.35) -- (0,2.35) node[above] {\(Y\)};

  \draw[thin] (0,0) -- (0.75,2.0);
  \draw[thin] (0,0) -- (1.95,1.45);
  \node[draw,align=center,minimum width=1.25cm] at (-1.2,1.45) {hệ số\\\(t_2\)};
  \node[draw,align=center,minimum width=1.25cm] at (1.55,1.85) {hệ số\\\(t_1\)};
  \draw[thin] (-1.05,1.05) -- (0.55,1.45);
  \draw[thin] (1.35,1.45) -- (0.48,0.92);

  \draw[red,thick] plot[smooth cycle] coordinates
    {(0.18,0.25) (0.55,0.82) (1.05,1.23) (1.55,1.4)
     (2.0,1.08) (2.12,0.72) (1.78,0.42) (1.15,0.26) (0.55,0.12)};
  \draw[red,thick] plot[smooth cycle] coordinates
    {(-0.12,-0.28) (-0.48,-0.88) (-0.88,-1.32) (-1.25,-1.7)
     (-0.65,-1.85) (-0.22,-1.38) (-0.06,-0.82)};
  \draw[red,thick] plot[smooth cycle] coordinates
    {(0.22,-0.32) (0.55,-0.82) (0.88,-1.48) (1.35,-1.26)
     (1.78,-0.65) (1.28,-0.38) (0.75,-0.18)};

  \foreach \p in {(0.58,0.62),(0.95,0.94),(1.42,0.72),(1.75,0.25)} {
    \draw[->,thin] (0,0) -- \p;
  }
  \foreach \p in {(-0.35,-1.18),(-0.72,-1.7)} {
    \draw[->,thin] (0,0) -- \p;
  }
  \foreach \p in {(0.6,-1.35),(1.1,-0.86)} {
    \draw[->,thin] (0,0) -- \p;
  }

  \node[draw,rounded corners=2pt,fill=white] at (-1.75,0.92) {\(Q_1\)};
  \node[draw,rounded corners=2pt,fill=white] at (-0.45,-2.05) {\(Q_2\)};
  \node[draw,rounded corners=2pt,fill=white] at (1.05,-2.05) {\(Q_3\)};
  \draw[thin] (-1.45,0.8) -- (0.76,0.74);
  \draw[thin] (-0.45,-1.83) -- (-0.52,-1.34);
  \draw[thin] (1.05,-1.83) -- (0.9,-1.3);
\end{tikzpicture}

\smallskip
\textit{Sơ đồ tách các nhóm vectơ trong chứng minh Kết luận 2.3.2.}
\end{center}
...
\begin{tikzpicture}[scale=0.85,every node/.style={font=\small}]
  \tikzset{v/.style={circle,fill=black,inner sep=2.2pt,label={[font=\small]#1}}}
  \node[v=left:\(A\)] (A) at (0,2.4) {};
  \node[v=above:\(B\)] (B) at (2.35,1.85) {};
  \node[v=above:\(C\)] (C) at (3.25,3.1) {};
  \node[v=right:\(D\)] (D) at (4.45,1.35) {};
  \node[v=below:\(E\)] (E) at (1.6,0.2) {};
  \node[v=left:\(F\)] (F) at (-1.15,1.0) {};
  \node[v=below:\(G\)] (G) at (-0.9,-0.85) {};
  \draw (A)--(B)--(C)--(D)--(B)--(E)--(F)--(G)--(E);
\end{tikzpicture}

\smallskip
\textit{Đồ thị ví dụ của trò chơi bàn cờ; các vị trí thắng là \(A,E,F,G\).}
\end{center}

Từ hình có thể đọc được phần lập luận cần thiết: nếu Giáp đặt quân ở \(A\),
Ất chỉ có thể đưa quân tới \(B\). Khi đó Giáp đưa quân tới \(E\). Nếu Ất tiếp
tục đi tới \(G\), Giáp đưa quân tới \(F\); nếu Ất đi tới \(F\), Giáp đưa quân
tới \(G\). Trong cả hai trường hợp, Ất đều hết đường đi, nên Giáp có chiến
lược thắng.
...
lớn nhất ấy, mọi đỉnh thuộc \(A\) không được ghép đều là đỉnh thắng. Trong
hình minh họa dưới đây, cạnh đỏ là ghép cặp lớn nhất, đỉnh xanh lá là vị trí
hiện tại, đỉnh xám là đỉnh đã đi qua, và các mũi tên biểu diễn một phần cây
chiến lược của Giáp.

\begin{center}
\begin{tikzpicture}[>=Latex,scale=0.72,every node/.style={font=\scriptsize}]
  \tikzset{
    bnode/.style={circle,draw=black,fill=blue!85,inner sep=2.2pt},
    gnode/.style={circle,draw=black,fill=green!80!black,inner sep=2.2pt},
    xnode/.style={circle,draw=gray,fill=gray!35,inner sep=2.2pt},
    match/.style={red,line width=1.3pt},
    edge/.style={thin},
    flowarrow/.style={->,line width=1.2pt,gray!70}
  }
  \newcommand{\mini}[5]{%
    \begin{scope}[shift={(#2,#3)}]
      \foreach \i/\style in {1/bnode,2/bnode,3/bnode,4/#4,5/bnode}
        \node[\style] (L#1-\i) at (0,5-\i) {};
      \foreach \i/\style in {1/bnode,2/#5,3/bnode,4/bnode,5/bnode}
        \node[\style] (R#1-\i) at (1.05,5-\i) {};
      \draw[edge] (L#1-1)--(R#1-2);
      \draw[edge] (L#1-2)--(R#1-1);
      \draw[edge] (L#1-3)--(R#1-2);
      \draw[edge] (L#1-4)--(R#1-2);
      \draw[edge] (L#1-4)--(R#1-3);
      \draw[edge] (L#1-4)--(R#1-4);
      \draw[match] (L#1-1)--(R#1-2);
      \draw[match] (L#1-2)--(R#1-1);
      \draw[match] (L#1-3)--(R#1-4);
      \draw[match] (L#1-5)--(R#1-5);
    \end{scope}}
  \mini{a}{0}{0}{gnode}{bnode}
  \mini{b}{2.5}{0}{xnode}{gnode}
  \mini{c}{5.0}{0}{bnode}{xnode}
  \mini{d}{7.5}{0}{bnode}{gnode}
  \mini{e}{2.5}{-4.2}{xnode}{bnode}
  \mini{f}{5.0}{-4.2}{gnode}{xnode}
  \mini{g}{7.5}{-4.2}{gnode}{bnode}
  \draw[flowarrow] (1.55,2.0) -- (2.15,2.0);
  \draw[flowarrow] (4.05,2.0) -- (4.65,2.0);
  \draw[flowarrow] (6.55,2.0) -- (7.15,2.0);
  \draw[flowarrow] (8.05,-0.1) -- (8.05,-1.15);
  \draw[flowarrow] (1.0,-0.55) -- (2.25,-2.35);
  \draw[flowarrow] (4.05,-2.25) -- (4.65,-2.25);
\end{tikzpicture}

\smallskip
\textit{Một phần cây chiến lược khi đi theo một ghép cặp lớn nhất.}
\end{center}
