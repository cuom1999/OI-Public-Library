...
\begin{figure}[ht]
\centering
\begin{tikzpicture}[>=Latex, every node/.style={font=\small}, x=0.72cm, y=0.55cm]
\draw[->] (0,0) -- (8.4,0) node[right] {cột};
\draw[->] (0,0) -- (0,5.2) node[above] {chiều cao};
\foreach \x/\h in {1/2,2/4,3/4,4/3,5/5,6/2,7/1} {
  \draw[fill=gray!18] (\x,0) rectangle ++(0.55,\h);
}
\filldraw[blue!70!black] (1.2,2) circle (2pt) node[above left] {\((2,1)\)};
\filldraw[blue!70!black] (2.2,4) circle (2pt) node[above] {\((4,2)\)};
\filldraw[blue!70!black] (4.2,3) circle (2pt) node[above] {\((3,4)\)};
\draw[red!70!black, dashed] (6,0) -- (6,5) node[above] {cột \(i\)};
\draw[red!70!black, ->, thick] (5.0,4.8) -- (4.35,3.15);
\draw[red!70!black, ->, thick] (3.0,4.5) -- (2.25,4.12);
\node[align=center] at (4.5,-1.05) {Khi gặp chiều cao nhỏ hơn ở cột hiện tại,\\
các điểm cao hơn bị lấy khỏi ngăn xếp và tạo ứng viên diện tích.};
\end{tikzpicture}
\caption{Ngăn xếp đơn điệu cho bài hình chữ nhật toàn số không: mỗi điểm lưu
\((\text{chiều cao},\text{cột trái nhất})\).}
\end{figure}
...
\begin{figure}[ht]
\centering
\begin{tikzpicture}[>=Latex, every node/.style={font=\small}, x=0.78cm, y=0.48cm]
\draw[->] (0,0) -- (8.5,0) node[right] {chỉ số};
\draw[->] (0,0) -- (0,6.0) node[above] {giá trị \(a_i\)};
\foreach \x/\y/\lab in {1/2/a_1,2/4/a_2,3/3/a_3,4/5/a_4,5/1/a_5,6/4/a_6,7/2/a_7} {
  \filldraw[black] (\x,\y) circle (2pt) node[above] {\(\lab\)};
}
\draw[blue!70!black, thick] (2,4) -- (4,5) -- (6,4) -- (7,2);
\draw[red!70!black, ->, thick] (1,2) -- (3.7,4.75);
\draw[red!70!black, ->, thick] (3,3) -- (3.8,4.85);
\node[red!70!black, align=center] at (2.4,1.0) {bị \(a_4\) loại};
\draw[orange!85!black, thick] (3.8,-0.25) -- (6.2,-0.25);
\node[align=center] at (4.95,-1.05) {Trong cửa sổ hiện tại, hàng đợi chỉ giữ\\
các ứng viên giảm dần từ đầu tới cuối.};
\end{tikzpicture}
\caption{Hàng đợi đơn điệu trong bài \textit{Điệu valse lộng lẫy}: một điểm
thấp hơn nằm trước điểm cao hơn ở bên phải sẽ không bao giờ trở thành cực đại.}
\end{figure}
