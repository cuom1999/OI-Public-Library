...
\newcommand{\assetbase}{assets/ioi/2009/li-jiyang-segment-skip-list}
\newcommand{\figimg}[4][0.88]{%
  \begin{center}
  \includegraphics[width=#1\linewidth]{\assetbase/#3}\\[-0.2em]
  {\small Hình #2. #4}
  \end{center}
}
...
\figimg[0.72]{1.1}{fig-1-1-linked-list-shortcut.png}{Danh sách liên kết có
con trỏ tăng tốc: ngoài các cạnh nối tiếp thông thường còn có vài cạnh dài
giúp bỏ qua nhiều nút.}
...
\figimg[0.82]{1.2}{fig-1-2-imaginary-skip-list.png}{Một ``danh sách nhảy
tưởng tượng'': các con trỏ tăng tốc nối tới các nút giữa của những đoạn có
thể xuất hiện trong quá trình tìm kiếm nhị phân.}
...
\figimg[0.82]{1.3}{fig-1-3-skip-list.png}{Một danh sách nhảy gồm nhiều tầng
danh sách liên kết; các bản sao của cùng một khóa được nối theo chiều dọc
thành một khối.}
...
\figimg[0.84]{1.4}{fig-1-4-search-path.png}{Một quá trình tìm kiếm: đường màu
xanh là đường tìm kiếm, các cạnh màu đỏ là những cạnh đã thử so sánh nhưng
không đi qua.}
...
\figimg[0.84]{1.5.1}{fig-1-5-1-before-insert.png}{Trạng thái trước khi chèn:
các cạnh đỏ chỉ vị trí trên từng tầng nơi nút mới sẽ được chen vào.}

\figimg[0.84]{1.5.2}{fig-1-5-2-after-insert.png}{Trạng thái sau khi chèn: nút
mới xuất hiện trên các tầng đã chọn ngẫu nhiên, đồng thời các cạnh dọc nối các
bản sao của nó.}
...
\figimg[0.84]{1.6.1}{fig-1-6-1-delete-search.png}{Trước tiên tìm các nút cần
xóa.}

\figimg[0.86]{1.6.2}{fig-1-6-2-delete-relink.png}{Xóa các nút và cạnh màu đỏ,
rồi nối lại các cạnh màu xanh để khôi phục các danh sách liên kết từng tầng.}
...
\figimg[0.82]{2.1}{fig-2-1-skip-list.png}{Một danh sách nhảy thông thường.}
...
\figimg[0.82]{2.2}{fig-2-2-bst-in-skip-list.png}{Cây tìm kiếm nhị phân ẩn
trong danh sách nhảy sau khi chỉ giữ các cạnh thật.}
...
\figimg[0.86]{2.3.1}{fig-2-3-1-segment-predecessor.png}{Tiền thân của danh
sách nhảy đoạn: các điểm rời rạc được đánh dấu dọc theo các đường tìm kiếm.}
...
\figimg[0.86]{2.3.2}{fig-2-3-2-final-segment-skip-list.png}{Phiên bản cuối
cùng của danh sách nhảy đoạn: một cây đoạn ẩn trong danh sách nhảy, có thêm
tầng 0 để biểu diễn các lá.}
...
\figimg[0.86]{2.4.1}{fig-2-4-1-insert-affected-ranges.png}{Sự thay đổi các
đoạn do nút SSL biểu diễn trong quá trình chèn. Các nút viền xanh là nút mới
thêm; các đoạn chữ đỏ là những đoạn bị thay đổi. Ngoài một phần của
\(\Path(v)\) và \(\Path(v-\varepsilon)\), đoạn tương ứng với các nút khác
không đổi.}
