...
đoạn đang xét; hình trong bản gốc minh họa rằng các hạng dần tách ra cho tới
khi mọi hậu tố có hạng riêng.
...
trên đoạn quyết định độ dài tiền tố chung của hai đầu mút. Hình trong bản gốc
minh họa điều này với xâu \texttt{aabaaaab}, khi hỏi LCP của hai hậu tố
\texttt{abaaaab} và \texttt{aaab}.
...
truy vấn LCP tương ứng với từng tâm. Hình trong bản gốc mô tả việc phản chiếu
hai nửa qua tâm trong xâu ghép này. Độ phức tạp là \(\Oh(n\log n)\), hoặc
\(\Oh(n)\) nếu \(\RMQ\) được tiền xử lý tuyến tính.
...
cặp hậu tố kề nhau trong \(\SA\). Chỉ khi \(\SA[i-1]\) và \(\SA[i]\) thuộc
