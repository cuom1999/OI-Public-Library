...
\newcommand{\TriangleConstructionGrid}[1]{%
\begin{tikzpicture}[x=0.9cm,y=0.9cm,line join=round,line cap=round]
  \def\hh{0.866}
  \foreach \r in {0,...,3} {
    \pgfmathsetmacro{\y}{\hh*\r}
    \pgfmathsetmacro{\yt}{\hh*(\r+1)}
    \foreach \i in {0,...,3} {
      \ifnum\i<\numexpr4-\r\relax
        \draw[fill=white] ({\i+0.5*\r},\y) --
          ({\i+1+0.5*\r},\y) --
          ({\i+0.5+0.5*\r},\yt) -- cycle;
      \fi
    }
  }
  \foreach \r in {0,...,2} {
    ...
    \foreach \i in {0,...,2} {
      \ifnum\i<\numexpr3-\r\relax
        \draw[fill=#1] ({\i+0.5+0.5*\r},\yt) --
          ({\i+1.5+0.5*\r},\yt) --
          ({\i+1+0.5*\r},\y) -- cycle;
      \fi
    }
  }
  \draw[thick] (0,0) -- (4,0) -- (2,{4*\hh}) -- cycle;
\end{tikzpicture}%
}
...
bài viết thảo luận các cách tối ưu chủ yếu dùng chung
...
Các tam giác nhỏ được ghép lại sao cho hai cạnh kề nhau phải có cùng màu, như
trong lưới tam giác minh họa dưới đây. Bài toán cho màu của ba cạnh của từng
tam giác trong $N^2$ tam giác nhỏ; hãy xác định có thể dùng chúng để tạo thành
một tam giác đều cạnh $N$ hay không.

\begin{center}
\TriangleConstructionGrid{white}

\smallskip
\textit{Ví dụ lưới tam giác đều cạnh \(4\), gồm \(16\) tam giác nhỏ.}
\end{center}
...
Với bài này, ta có thể liệt kê sáu tam giác hướng xuống được tô bóng trong
hình dưới; mười tam giác còn lại đều có thể dựng thành quan hệ hai phía đơn
giản.

\begin{center}
\TriangleConstructionGrid{gray!35}

\smallskip
\textit{Sáu tam giác tô bóng là phần được tìm kiếm trực tiếp.}
\end{center}
...
\begin{itemize}
  ...
  \item NOI 2003, \textit{Phá trận liên hoàn}.
\end{itemize}
