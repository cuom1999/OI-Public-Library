...
\node[v] (s2) at (0,0) {$S_2$};
\node[v] (s) at (1.6,0) {$S$};
\node[v] (p1) at (3.4,1.1) {$1$};
\node (pdots) at (3.4,0) {$\vdots$};
\node[v] (pn) at (3.4,-1.1) {$N$};
\node[v] (q1) at (5.5,1.1) {$N+1$};
\node (qdots) at (5.5,0) {$\vdots$};
\node[v] (qn) at (5.5,-1.1) {$2N$};
\node[v] (t) at (7.2,0) {$T$};
\draw[arc] (s2) -- node[above] {$M$} (s);
\draw[arc] (s) -- node[above left] {$C_1$} (p1);
\draw[arc] (s) -- node[below left] {$C_N$} (pn);
\draw[arc,blue!65!black] (p1) -- node[above] {$1$} (q1);
\draw[arc,blue!65!black,densely dashed] (p1) -- (qn);
\draw[arc,blue!65!black,densely dashed] (pn) -- (q1);
\draw[arc,blue!65!black] (pn) -- node[below] {$1$} (qn);
\draw[arc] (q1) -- node[above right] {$D_1$} (t);
\draw[arc] (qn) -- node[below right] {$D_N$} (t);
\end{tikzpicture}
\caption{Mạng hai phía của bài đua xe: mỗi cung xanh biểu diễn một cặp xe mà P thắng Q.}
\end{figure}
...
\begin{figure}[ht]
\centering
\begin{tikzpicture}[x=.82cm,y=.76cm,>=Stealth,every node/.style={font=\small}]
\draw[->] (1.4,0) -- (8.5,0) node[right] {thời gian};
\foreach \x in {2,...,8} {
  \draw (\x,.06) -- (\x,-.06) node[below] {\x};
}
\draw[thick] (1.7,2) -- (8.2,2) node[right] {ray 1};
\draw[thick] (1.7,1) -- (8.2,1) node[right] {ray 2};
\draw[fill=gray!12] (2,2.08) rectangle (4,2.5) node[midway] {$1:0.05$};
\draw[fill=gray!12] (5,2.08) rectangle (8,2.5) node[midway] {$3:0.06$};
\draw[fill=gray!12] (3,1.08) rectangle (5,1.5) node[midway] {$2:0.04$};
\draw[fill=gray!12] (6,1.08) rectangle (8,1.5) node[midway] {$5:0.03$};
\draw[densely dashed] (2,.25) rectangle (4,.63) node[midway] {$4:0.02$};
\node[anchor=west] at (4.15,.44) {không chọn};
\end{tikzpicture}
\caption{Một lịch đạt lợi nhuận \(0.18\): các tàu \(1,3\) dùng ray thứ nhất, các tàu \(2,5\) dùng ray thứ hai.}
\end{figure}
...
\node[v] (s) at (1.4,0) {$S$};
\node[v] (i) at (3,0.8) {$i$};
\node[v] (ip) at (4.4,0.8) {$i'$};
\node[v] (j) at (3,-0.8) {$j$};
\node[v] (jp) at (4.4,-0.8) {$j'$};
\node[v] (t) at (6.2,0) {$T$};
\draw[arc] (sp) -- node[above] {$n$} (s);
\draw[arc] (s) -- node[above left] {$1$} (i);
\draw[arc] (s) -- node[below left] {$1$} (j);
\draw[arc,blue!65!black] (i) -- node[above] {$c_i$} (ip);
\draw[arc,blue!65!black] (j) -- node[below] {$c_j$} (jp);
\draw[arc] (ip) -- node[right] {$0$} (j);
\draw[arc] (ip) -- (t);
\draw[arc] (jp) -- (t);
\end{tikzpicture}
\caption{Mỗi tàu được tách thành \(i\to i'\), với \(c_i=1\%\cdot\mathrm{Cost}_i\); cung \(i'\to j\) biểu diễn thứ tự tương thích trên cùng một ray.}
\end{figure}
...
\node[v] (d4) at (3,-1) {$8$};
\node[v] (t) at (4.5,-1) {$10$};
\draw[arc] (s) -- node[above] {$f=3$} (u1);
\draw[arc] (u1) -- (u2);
\draw[arc] (u2) -- (u3);
\draw[arc] (u3) -- (u4);
\draw[arc,blue!65!black] (u1) -- node[left] {$8$} (d1);
\draw[arc,blue!65!black] (u2) -- node[left] {$2$} (d2);
\draw[arc,blue!65!black] (u3) -- node[left] {$1$} (d3);
\draw[arc,blue!65!black] (u4) -- node[left] {$6$} (d4);
\draw[arc] (d1) -- (t);
\draw[arc] (d2) -- (t);
\draw[arc] (d3) -- (t);
\draw[arc] (d4) -- (t);
\draw[arc,red!70!black,bend left=18] (d1) to node[above] {$f_a=2$} (u3);
\draw[arc,red!70!black,bend left=18] (d2) to node[above] {$f_a=2$} (u4);
\draw[arc,green!45!black,bend right=18] (d1) to node[below] {$f_b=1$} (u4);
\end{tikzpicture}
\caption{Ví dụ trong slide với \(n=4,a=1,b=2,f=3,f_a=2,f_b=1\) và nhu cầu \(8,2,1,6\).}
\end{figure}
